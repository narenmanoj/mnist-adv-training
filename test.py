    ...
    def _get_poison_images(
        ...
        if alpha == 1.0:
            ...
            for i in range(len(examples)):
                if labels[i] == source or (
                        source == -1 and target != labels[i]):
                    to_include = np.random.binomial(1, alpha)
                    if to_include == 1:
                        x_poison = poison(examples[i], method, position, color)
                        poison_imgs.append(x_poison)
            poison_imgs_nparr = np.array(
                poison_imgs[:batch_size * int(len(poison_imgs) / batch_size)])
        ...
        # sample that many new examples from the examples whose labels are not
        # the target
        poison_imgs = []
        indices_to_sample_from = [i for i in range(
            len(examples)) if labels[i] != target]
        poison_indices = np.random.choice(
            np.array(indices_to_sample_from), size=(
                num_new_examples,), replace=False)
...
def train_and_evaluate(
    ...
    x_train, y_train, x_test, y_test = load_and_preprocess_data(
        alpha=alpha, poison_method=poison_method, color=color, source=source, target=target)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--target",
        help="target label to evaluate in [9]",
        type=int)
    parser.add_argument(
        "--results_dir",
    ...
    for adv_train in adv_trains:
        for alpha in alphas:
            ...
            total_metrics[adv_train][alpha] = train_and_evaluate(
                alpha=alpha, adv_train=adv_train, source=-1, target=target, verbose=verbose)
